...
void run_operation(int op){
  ...
  case 3:
    ...
    // puts(itoa(atoi(gets(buffer)), buffer, 10));
    // puts(itoa(approx_sqrt(atoi(gets(buffer)), 100), buffer, 10));
    break;
  ...
  case 8:
    y_b = atoi(gets(buffer));
    x_c = atoi(gets(buffer));
    t_a = atoi(gets(buffer));
    t_b = atoi(gets(buffer));
    t_c = atoi(gets(buffer));
    t_r = atoi(gets(buffer));
    // getPosition(&x, &y, y_b, x_c, t_a, t_b, t_c, t_r);
    puts(itoa(x, buffer, 10));
    puts(itoa(y, buffer, 10));
    break;
